...
namespace CasabaSecurity.Web.Watcher.Checks
{
    ...
    public class CheckPasvUnicodeInvalidUTF8 : WatcherCheck
    {
        ...
        private void AddAlert2(Session session)
        {
            ...
            WatcherEngine.Results.Add(WatcherResultSeverity.High, session.id, session.url, name, text, StandardsCompliance, findingnum2);
        }
        ...
        private void FormMessage(long position, int sequencelength, MemoryStream memstream, bool isHeader)
        {
            ...
            if (isHeader)
            {
                message = message + findingnum.ToString() + ") An invalid " + sequencelength.ToString() + " character UTF-8 ByteStream was found in the header at byte position " + position.ToString() + "\r\n";
            }
            else
            {
                message = message + findingnum.ToString() + ") An invalid " + sequencelength.ToString() + " character UTF-8 ByteStream was found at byte position " + position.ToString() + "\r\n";
            }
        ...
        private void FormMessage2(long position, MemoryStream memstream, bool isHeader)
        {
            ...
            {
                message = message + findingnum2.ToString() + ") An invalid Null byte in ByteStream was found at byte position " + position.ToString() + "\r\n";
            }
        ...
        void scanStream(MemoryStream memstream, bool isheader)
        {
            byte firstbyte;
            byte secondbyte;
            byte thirdbyte;
            ...
            while (true)
            {
                ...
                {
                    FormMessage2(position, memstream, isheader);
                }
                ...
                {
                    FormMessage(position, returnvalue.goal, memstream, isheader);
                }
        ...
        {
            MemoryStream memstreamHeader;
            MemoryStream memstreamBody;
            alertbody = "";
            alertbody2 = "";
            findingnum = 0;
            findingnum2 = 0;
                ...
                if (session.responseCode == 200)
                {
                    if (Utility.IsResponseHtml(session) || Utility.IsResponseJavascript(session) || Utility.IsResponseXml(session))
                    {
                        if (Utility.IsResponseCharset(session, "utf-8") || Utility.IsResponseCharset(session, "utf8"))
                        {
                            ...
                            //Check Headers
                            if (session.oResponse.headers.ToByteArray(false, false) != null)
                            {
                                //Open Memory Stream
                                memstreamHeader = new MemoryStream(session.oResponse.headers.ToByteArray(false, false));
                                try
                                {
                                    scanStream(memstreamHeader, true);
                                }
                                catch { }
                            }
                            ...
                            {
                                //Open Memory Stream
                                memstreamBody = new MemoryStream(session.responseBodyBytes);
                                try
                                {
                                    scanStream(memstreamBody, false);
                                }
                                catch { }
                            }
